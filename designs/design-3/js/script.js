...
// The hero "Get a Quote" button jumps down to the quote bar. Both sit inside
// the same 708px hero - the button at y448, the bar at y564 - so the travel is
// only about 116px and on a tall viewport the bar may already be on screen.
// A bare jump would therefore look like nothing happened, so the arrival is
// carried by a flash on the bar and by putting the caret in the pick-up field,
// which is what the visitor came to fill in anyway.
function initHeroQuoteJump() {
  var cta = document.querySelector('.hero-quote-cta');
  var section = document.getElementById('quote-bar-section');
  ...
  cta.addEventListener('click', function (e) {
    ...
    // restart the flash rather than let a repeat click land on a class that is
    // already applied and so animates nothing
    section.classList.remove('is-targeted');
    void section.offsetWidth;
    section.classList.add('is-targeted');

    var input = section.querySelector('.quote-option.is-active input');
    if (!input) return;

    // focus after the scroll, and without stealing it back to the top: preventScroll
    // keeps the browser from re-scrolling to the field mid-animation
